 ...
 */
function looseCompareProps(oProps: Record<string, any>, eProps: Record<string, any>): boolean {
 ...
 */
export default function domWarn(): IUnifiedTransformer {
	return (ast) => {
		visit(ast, 'element', (node: IElmNode, ancestors) => {
			// only process the p elements below the root
			if (is(node, 'p') && ancestors[0].type === 'root' && ancestors.length === 1) {
				const nodes: IElmNode[] = []
				...
				if (nodes.length > 1 || !is(nodes[0], 'p')) {
					const parent = ancestors[ancestors.length - 1] as IElmNode
